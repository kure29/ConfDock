...
This development inventory remains descriptive and is never shipped as a
substitute for legal notices. The distribution artifact is
`THIRD_PARTY_NOTICES.md`, generated by the repository-owned, versioned
`scripts/generate-third-party-notices.mjs` from the two target-specific Cargo
normal graphs and the Web production lockfile records. Dual-license choices
are explicit; unknown, missing, or unapproved expressions fail closed. CI
regenerates the file into a temporary directory and requires a byte-for-byte
match.

The first public Release remains blocked until the complete release dry-run,
including notice regeneration, production-bundle attribution inspection,
binary/image identity, Docker smoke, and SBOM checks, succeeds for the exact
main Commit. Merely committing the notices file does not create or approve a
Release.
